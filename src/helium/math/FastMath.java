...
public final class FastMath {
    ...
    public static void init(int lutSize) {
        precision = lutSize;
        precisionFactor = (double)lutSize / (Math.PI * 2);
        sinTable = new float[lutSize];
        cosTable = new float[lutSize];
        for (int i = 0; i < lutSize; ++i) {
            double angle = (double)i * Math.PI * 2.0 / (double)lutSize;
            FastMath.sinTable[i] = (float)Math.sin(angle);
            FastMath.cosTable[i] = (float)Math.cos(angle);
        }
        initialized = true;
    }
    ...
    public static void batchSin(float[] input, float[] output) {
        ...
        for (int i = 0; i < len; ++i) {
            int index = (int)((double)input[i] * precisionFactor) % precision;
            if (index < 0) {
                index += precision;
            }
    ...
    public static void batchSinCos(float[] angles, float[] sinOut, float[] cosOut) {
        if (!initialized || angles == null || sinOut == null || cosOut == null) {
            return;
        }
        int len = Math.min(angles.length, Math.min(sinOut.length, cosOut.length));
        for (int i = 0; i < len; ++i) {
            int index = (int)((double)angles[i] * precisionFactor) % precision;
            if (index < 0) {
                index += precision;
            }
    ...
    public static void batchTransformPositions(float[] positions, float[] matrix4x4, float[] output) {
        if (positions == null || matrix4x4 == null || output == null) {
            return;
        }
    ...
    public static void batchNormalize(float[] vectors, float[] output) {
        ...
        for (int i = 0; i < vectorCount; ++i) {
            ...
            float invLen = (float)FastMath.inverseSqrt(x * x + y * y + z * z);
            output[idx] = x * invLen;
